<div class="tg-bot-card">
    ...
    <div class="tg-card-body">
        <div class="tg-info-box mb-3">
            ...
            <div class="text-white mb-1">🕒 <strong class="text-white">Last Check Time:</strong> <span class="text-white-50">{{ $autobuyState['last_check'] ?? 'Awaiting next cycle' }}</span></div>
            <div class="text-white">📊 <strong class="text-white">Checks Logged:</strong> <span class="badge bg-primary">{{ $autobuyState['total_checks'] ?? 0 }}</span> &bull; <strong class="text-white">Accounts Secured:</strong> <span class="badge bg-success">{{ $autobuyState['total_bought'] ?? 0 }}</span></div>
        </div>

        <!-- Live Activity Log Table -->
        <div class="mb-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <h6 class="text-white fw-bold mb-0" style="font-size: 13px;">📋 Recent Auto-Buy Activity Feed:</h6>
                    ...
                    <table class="tg-custom-table w-100">
                        ...
                        <tbody>
                            @foreach($checks as $chk)
                                <tr>
                                    <td class="text-white-50 small" style="font-size: 11px;">{{ $chk['time'] ?? '--' }}</td>
            ...
            @else
                <div class="tg-alert-info py-2 mb-0 small">
                    ⏳ Cron job is registered. Once the 30-second cron fires, live check logs will appear here.
                </div>
            @endif
        </div>
